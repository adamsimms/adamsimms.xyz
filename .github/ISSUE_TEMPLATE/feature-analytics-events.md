...
- No analytics is currently implemented on the site
- Cloudflare Web Analytics is the recommended option when adding tracking (see DEPLOYMENT.md)
...
document
  .querySelectorAll('a[href*="trello"], a[href*="atlassian"], a[href*="lightspeed"]')
  .forEach((link) => {
    link.addEventListener('click', () => {
      gtag('event', 'click', {
